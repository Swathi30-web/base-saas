...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setFormError(err.message || 'Unable to log in. Is JSON Server running?');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
      ...
      <div className="mb-5 bg-primary-50 text-primary-700 text-xs rounded-xl p-3.5 leading-relaxed">
        <p className="font-semibold mb-1">Demo accounts (JSON Server)</p>
        <p>admin@base.com / admin123 — Admin</p>
        <p>hr@base.com / hr123456 — HR</p>
        <p>employee@base.com / employee123 — Employee</p>
      </div>

      {formError && (
        <div className="mb-4 bg-rose-50 text-rose-600 text-sm rounded-xl px-4 py-3">{formError}</div>
      )}
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <Link to="/recover" className="text-primary-500 font-medium">Reset Password?</Link>
        </div>

        <button type="submit" disabled={submitting} className="btn-primary w-full disabled:opacity-60">
          {submitting ? <LoaderCircle size={18} className="animate-spin mx-auto" /> : 'Log in'}
        </button>

        <p className="text-center text-sm text-gray-600">
          Don't have account yet? <Link to="/signup" className="text-primary-500 font-medium">New Account</Link>
        </p>
      </form>
    </div>
  );
